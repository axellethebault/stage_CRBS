...
class SmartKMeans(sklearn.cluster.KMeans): #classe fille qui hérite de la classe KMeans de scikit, mais sans le paramètre n_cluster
    def __init__(self, init="k-means++", n_init="auto", max_iter=300, tol=1e-4, verbose=1, random_state=0, copy_x=True, algorithm="lloyd"):
        super().__init__(n_clusters=1,init=init, n_init=n_init, max_iter=max_iter, tol=tol, verbose=verbose, random_state=random_state) #PK N CLUSTER =1??
    ...
    def _find_best_model(self, data): #best_model = KMeans avec n_cluster optimisé
        ...
        max_range=20
        if max_range > data.size:  # On vérifie que il y a au moins 20 data, sinon on reduit le nombre de cluster maximal  
            max_range = data.size
            
        # Initialize the clusterer with n_clusters value and a random generator, seed of 0 for reproducibility.
        for n_clusters in range(2,max_range):
